...
export async function getMovieDetails(id: string) {
  try {
    const res = await fetch(
      `${BASE_URL}/${VERSION}/movie/${id}?append_to_response=videos%2Csimilar%2Ccredits%2C&language=en-US`,
      options
    );
    const data = await res.json();
    return data;
...
export async function getShowDetails(id: string) {
  try {
    const res = await fetch(
      `${BASE_URL}/${VERSION}/tv/${id}?append_to_response=videos%2Csimilar%2Ccredits%2Cseason%2F1%2Cseason%2F2%2Cseason%2F3%2C&language=en-US`,
      options
    );
    const data = await res.json();
    return data;
